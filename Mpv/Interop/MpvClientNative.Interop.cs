...
public partial class MpvClientNative
{
    ...
    //   必须由调用者用 mpv_free 释放。若声明为 string 返回值，LibraryImport 源生成器会在
    //   finally 块中调用 Utf8StringMarshaller.Free（内部即 Marshal.FreeHGlobal）释放该指针，
    //   与 mpv 的分配器不匹配 → 堆损坏 0xc0000374（ntdll.dll）。必须用 IntPtr 返回 + 手动释放。
    [LibraryImport(MpvIdentifier, StringMarshalling = StringMarshalling.Utf8)]
    ...
    // 释放 mpv_get_property 返回的 Node 类型属性（MPV_FORMAT_NODE/NODE_ARRAY/NODE_MAP）
    // 内部递归释放所有子节点；非 Node 类型（FLAG/INT64/DOUBLE/STRING）无需调用。
    [LibraryImport(MpvIdentifier)]
