...
namespace SightoHear
{
    ...
    {
        private FolderDetailArgs? _args;
        ...
        {
            InitializeComponent();
            Loaded += FolderDetailPage_Loaded;
            SizeChanged += FolderDetailPage_SizeChanged;
        }

        private void FolderDetailPage_Loaded(object sender, RoutedEventArgs e)
        {
            UpdateLayoutForCurrentWidth();
        }

        private void FolderDetailPage_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            UpdateLayoutForCurrentWidth();
        }
        ...
        private void UpdateLayoutForCurrentWidth()
        {
            ...
            // 更新歌曲列表列可见性
            UpdateSongListColumns(isNarrow);
        }

        /// <summary>
        /// 根据窗口宽度更新歌曲列表中的歌手列可见性。
        /// </summary>
        private void UpdateSongListColumns(bool isNarrow)
        {
            if (isNarrow)
            {
                // 极窄窗口：隐藏歌手列
                ArtistColumn2.Width = new GridLength(0);
                ArtistColumnHeader.Visibility = Visibility.Collapsed;
            }
            else
            {
                // 中等/宽窗口：正常显示
                ArtistColumn2.Width = new GridLength(160);
                ArtistColumnHeader.Visibility = Visibility.Visible;
            }
        ...
        private void UpdateSongListItemsColumns(bool isNarrow)
        {
            ...
            foreach (var container in SongList.Items)
            {
                if (SongList.ContainerFromItem(container) is ListViewItem lvi)
                {
                    ...
                    if (songItemGrid != null && songItemGrid.ColumnDefinitions.Count > 2)
                    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.Parameter is FolderDetailArgs args)
                _args = args;

            _songs = _args?.Songs ?? new List<MediaItem>();
            RefreshHeader();
            ApplyFilter();
            AppLogger.Info($"文件夹详情页打开: {_args?.FolderPath ?? "未知文件夹"} (歌曲 {_songs.Count} 首)");
        }

        private void RefreshHeader()
        {
            string name = _args != null ? Path.GetFileName(_args.FolderPath) : "文件夹";
            string fullPath = _args?.FolderPath ?? string.Empty;
            int songCount = _songs.Count;
            string statText = $"{songCount} 首歌";

            // 更新宽窗口布局
            WideFolderNameText.Text = name;
            WideFolderPathText.Text = fullPath;
            WideFolderStatText.Text = statText;
            WidePlayButton.IsEnabled = songCount > 0;

            // 更新中等窗口布局
            MediumFolderNameText.Text = name;
            MediumFolderPathText.Text = fullPath;
            MediumFolderStatText.Text = statText;
            MediumPlayButton.IsEnabled = songCount > 0;

            // 更新窄窗口布局
            NarrowFolderNameText.Text = name;
            NarrowFolderPathText.Text = fullPath;
            NarrowFolderStatText.Text = statText;
        ...
        {
            string coverPath = _args?.Songs.FirstOrDefault()?.FilePath ?? string.Empty;
            ...
            WideFolderCoverImage.Source = bitmap;
            MediumFolderCoverImage.Source = bitmap;
            NarrowFolderCoverImage.Source = bitmap;
        }
        ...
        private void FolderSongCheckBox_Checked(object sender, RoutedEventArgs e)
        {
        ...
        private void FolderSongCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (sender is CheckBox cb && cb.DataContext is MediaItem item)
            {
        ...
        private async Task PlaySongAsync(MediaItem item)
        {
            var queue = _filteredSongs.Count > 0
                ...
                : _songs.ToList();
            AppLogger.Info($"文件夹详情页播放: {item.Title} (队列 {queue.Count} 首)");
    ...
    public sealed class FolderDetailArgs
    {
