...
namespace SightoHear
{
    ...
    {
        private ArtistDetailArgs? _args;
        ...
        {
            InitializeComponent();
            Loaded += ArtistDetailPage_Loaded;
            SizeChanged += ArtistDetailPage_SizeChanged;
        }
        ...
        private void ArtistDetailPage_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            UpdateLayoutForCurrentWidth();
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.Parameter is ArtistDetailArgs args)
                _args = args;

            _songs = _args?.Songs ?? new List<MediaItem>();
            RefreshHeader();
            ApplyFilter();
            AppLogger.Info($"歌手详情页打开: {_args?.ArtistName ?? "未知歌手"} (歌曲 {_songs.Count} 首)");
        }

        private void RefreshHeader()
        {
            string name = _args?.ArtistName ?? "未知歌手";
            int songCount = _songs.Count;
            int albumCount = _songs.Select(m => m.AlbumDisplay).Distinct().Count();
            string statText = $"{songCount} 首歌 · {albumCount} 张专辑";

            // 更新宽窗口布局
            WideArtistNameText.Text = name;
            WideArtistStatText.Text = statText;
            WidePlayButton.IsEnabled = songCount > 0;

            // 更新中等窗口布局
            MediumArtistNameText.Text = name;
            MediumArtistStatText.Text = statText;
            MediumPlayButton.IsEnabled = songCount > 0;

            // 更新窄窗口布局
            NarrowArtistNameText.Text = name;
            NarrowArtistStatText.Text = statText;
        ...
        private void UpdateCoverImage(string coverPath)
        {
            ...
            WideArtistCoverImage.Source = bitmap;
            MediumArtistCoverImage.Source = bitmap;
            NarrowArtistCoverImage.Source = bitmap;
        }
        ...
        private void ArtistSongCheckBox_Checked(object sender, RoutedEventArgs e)
        {
        ...
        private void ArtistSongCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (sender is CheckBox cb && cb.DataContext is MediaItem item)
            {
        ...
        private async Task PlaySongAsync(MediaItem item)
        {
            var queue = _filteredSongs.Count > 0
                ...
                : _songs.ToList();
            AppLogger.Info($"歌手详情页播放: {item.Title} (队列 {queue.Count} 首)");
    ...
    public sealed class ArtistDetailArgs
    {
